...
const printLookup = data => {
  ...
  fmt.printSuccessLine(1, fmt.pad('IP ADDR:', 12), data.address);
...
const printNSRecords = data => {
  ...
  data.forEach((r, i) => {
    fmt.printSuccessBullet(1, '[+]', fmt.pad('HOSTNAME:', 12), r.hostname);

    // Print each possible address
    r.geo.forEach(h => {
      fmt.printSuccessLine(2, fmt.pad('IP ADDR:', 12), h.address);
      fmt.printSuccessLine(2, fmt.pad('GEO:', 12), formatGeo(h.geo.ll));
      ...
      fmt.printSuccessLine(2, fmt.pad('MAP:', 12), getMapLink(h.geo.ll));
    });
...
const printMXRecords = data => {
  ...
  _.orderBy(data, 'priority', ['desc']).forEach((mx, i) => {
    // Print exchange name
    fmt.printSuccessBullet(1, `[${++i}]`, fmt.pad('EXCHANGE:'), mx.exchange);
    fmt.printSuccessLine(2);

    // Print each possible address
    mx.geo.forEach((h, j) => {
      fmt.printSuccessBullet(2, `[${++j}]`, fmt.pad('IP ADDR:'), h.address);
      fmt.printSuccessLine(3, fmt.pad('GEO:'), formatGeo(h.geo.ll));
      fmt.printSuccessLine(
        3,
        fmt.pad('LOCATION:'),
        formatLocation({
